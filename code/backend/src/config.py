...
class BaseConfig:
    ...
    # Feature Flags
    FEATURE_BLOCKCHAIN_INTEGRATION = os.getenv('FEATURE_BLOCKCHAIN_INTEGRATION', 'true').lower() == 'true'
    FEATURE_ADVANCED_ANALYTICS = os.getenv('FEATURE_ADVANCED_ANALYTICS', 'true').lower() == 'true'
    FEATURE_AUTOMATED_COMPLIANCE = os.getenv('FEATURE_AUTOMATED_COMPLIANCE', 'true').lower() == 'true'
    FEATURE_REAL_TIME_NOTIFICATIONS = os.getenv('FEATURE_REAL_TIME_NOTIFICATIONS', 'true').lower() == 'true'
    ...
    def validate_config(cls):
        ...
        if cls.FEATURE_BLOCKCHAIN_INTEGRATION and not cls.WEB3_PROVIDER_URL:
            errors.append("WEB3_PROVIDER_URL is required when blockchain integration is enabled")
    ...
    def init_app(cls, app):
        ...
        for contract in required_contracts:
            if cls.FEATURE_BLOCKCHAIN_INTEGRATION and not getattr(cls, contract):
                raise ValueError(f"{contract} environment variable is required in production")
