...
from ..models.trading import Portfolio, PortfolioHolding, Trade, Order, TradeStatus
from ..models.carbon_credit import CarbonProject, CarbonCredit
...
class PortfolioService:
    ...
    def create_portfolio(self, user_id: int, portfolio_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for holding in holdings:
                ...
                holdings_data.append({
                    ...
                    'weight': float(current_value / total_value * 100) if total_value > 0 else 0
                })
    ...
    def optimize_portfolio(self, portfolio_id: int, optimization_params: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            target_return = optimization_params.get('target_return', np.mean(expected_returns))
            ...
            for i, asset in enumerate(available_assets):
                ...
                current_allocation = current_holding['weight'] / 100 if current_holding else 0
                
                recommendations.append({
                    'asset': asset,
                    'current_allocation': current_allocation,
                    'target_allocation': target_allocation,
                    'current_value': current_holding['current_value'] if current_holding else 0,
                    'target_value': target_value,
                    'rebalance_amount': target_value - (current_holding['current_value'] if current_holding else 0),
                    'action': self._determine_action(current_allocation, target_allocation)
                })
    ...
    def calculate_portfolio_performance(self, portfolio_id: int, period_days: int = 30) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Performance metrics
            total_return = (value_history[-1]['value'] / value_history[0]['value'] - 1) * 100
            ...
            attribution = self._calculate_performance_attribution(portfolio_id, period_days)
    ...
    def rebalance_portfolio(self, portfolio_id: int, target_allocations: Dict[str, float]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for asset, target_pct in target_allocations.items():
                ...
                current_holding = current_holdings.get(asset, {'current_value': 0, 'quantity': 0})
            ...
            return {
                ...
                'estimated_total_cost': sum(trade['estimated_cost'] for trade in execution_results),
                'status': 'initiated',
                'timestamp': datetime.utcnow().isoformat()
            }
    ...
    def get_portfolio_analytics(self, portfolio_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Performance attribution
            performance_attribution = self._calculate_performance_attribution(portfolio_id, 30)
            ...
            return {
                ...
                'recommendations': self._generate_portfolio_recommendations(portfolio, holdings),
                'timestamp': datetime.utcnow().isoformat()
            }
    ...
    def _calculate_portfolio_risk_metrics(self, portfolio: Portfolio, holdings: List[PortfolioHolding]) -> Dict[str, Any]:
    ...
    def _calculate_allocation_breakdown(self, holdings_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _get_available_assets(self, optimization_params: Dict[str, Any]) -> List[str]:
        ...
        return ['VCS_FORESTRY', 'GOLD_STANDARD_RENEWABLE', 'CDM_METHANE', 'CAR_FORESTRY']
    ...
    def _optimize_weights(self, expected_returns: np.ndarray, cov_matrix: np.ndarray, 
                         target_return: float, risk_tolerance: str) -> np.ndarray:
    ...
    def _calculate_efficient_frontier(self, expected_returns: np.ndarray, cov_matrix: np.ndarray) -> List[Dict[str, float]]:
        ...
        for target_return in target_returns:
            try:
                weights = self._optimize_weights(expected_returns, cov_matrix, target_return, 'moderate')
    ...
    def _get_current_holding(self, portfolio_id: int, asset: str) -> Optional[Dict[str, Any]]:
    ...
    def _determine_action(self, current_allocation: float, target_allocation: float) -> str:
    ...
    def _get_portfolio_value_history(self, portfolio_id: int, days: int) -> List[Dict[str, Any]]:
        """Get portfolio value history"""
    ...
    def _calculate_annualized_return(self, returns: List[float], period_days: int) -> float:
    ...
    def _calculate_performance_attribution(self, portfolio_id: int, days: int) -> Dict[str, Any]:
    ...
    def _get_current_holdings_dict(self, portfolio_id: int) -> Dict[str, Dict[str, Any]]:
    ...
    def _analyze_diversification(self, holdings: List[PortfolioHolding]) -> Dict[str, Any]:
        """Analyze portfolio diversification"""
        if not holdings:
            return {'score': 0, 'recommendations': ['Add holdings to improve diversification']}
        ...
        diversification_score = min(100, (unique_projects * 10 + unique_types * 15 + unique_vintages * 5))
    ...
    def _analyze_portfolio_risks(self, portfolio: Portfolio, holdings: List[PortfolioHolding]) -> Dict[str, Any]:
    ...
    def _generate_portfolio_recommendations(self, portfolio: Portfolio, holdings: List[PortfolioHolding]) -> List[str]:
        ...
        total_value = sum(float(h.current_value or 0) for h in holdings)
        if total_value > 0:
            max_weight = max(float(h.current_value or 0) / total_value for h in holdings)
            if max_weight > 0.3:
                recommendations.append('Consider reducing concentration in largest position')
        
        recommendations.append('Regular rebalancing recommended to maintain target allocations')
