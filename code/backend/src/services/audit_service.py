...
class AuditService:
    ...
    def __init__(self):
        self.enabled = current_app.config.get('AUDIT_LOG_ENABLED', True) if current_app else True
    ...
    def log_event(self, event_type: str, event_category: str, event_description: str,
                  ...
                  session_id: Optional[str] = None, old_values: Optional[Dict] = None,
                  new_values: Optional[Dict] = None, metadata: Optional[Dict] = None,
        ...
        try:
            ...
            logger.log(log_level, f"AUDIT: {event_category}.{event_type} - {event_description} "
    ...
    def log_authentication_event(self, event_type: str, user_id: Optional[int] = None,
    ...
    def log_trading_event(self, event_type: str, user_id: int, trade_data: Dict,
                         success: bool = True, error_message: Optional[str] = None) -> bool:
        """
    ...
    def log_compliance_event(self, event_type: str, user_id: Optional[int] = None,
    ...
    def log_kyc_event(self, event_type: str, user_id: int, kyc_data: Dict,
                     success: bool = True, error_message: Optional[str] = None) -> bool:
        """
    ...
    def log_data_change(self, event_type: str, user_id: int, table_name: str,
                       record_id: int, old_values: Dict, new_values: Dict,
                       success: bool = True, error_message: Optional[str] = None) -> bool:
        """
    ...
    def get_user_activity_summary(self, user_id: int, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            ).group_by(UserAuditLog.event_category).all()
            
            events_by_category = {row.event_category: row.count for row in category_query}
            ...
            # Get recent events
            recent_events = UserAuditLog.query.filter(
                and_(
                    UserAuditLog.user_id == user_id,
                    UserAuditLog.created_at >= start_date
                )
            ).order_by(desc(UserAuditLog.created_at)).limit(10).all()
    ...
    def get_security_events(self, user_id: Optional[int] = None,
        ...
        try:
            ...
            security_event_types = [
                'login_failed', 'login_blocked', 'mfa_failed', 'password_change_failed',
                'suspicious_activity', 'account_locked', 'unauthorized_access'
            ]
            ...
            security_events = query.order_by(desc(UserAuditLog.created_at)).limit(100).all()
    ...
    def cleanup_old_logs(self, retention_days: Optional[int] = None) -> int:
        ...
        try:
            if retention_days is None:
                retention_days = current_app.config.get('DATA_RETENTION_DAYS', 2555)  # 7 years default
            ...
            if deleted_count > 0:
                ...
                self.log_system_event(
                    ...
                    metadata={'deleted_count': deleted_count, 'retention_days': retention_days}
                )
    ...
    def _sanitize_kyc_data(self, data: Dict) -> Dict:
        ...
        sensitive_fields = [
            'document_number', 'ssn', 'tax_id', 'passport_number',
            'drivers_license_number', 'bank_account_number'
        ]
        sanitized = data.copy()
    ...
    def _sanitize_data_values(self, data: Dict) -> Dict:
        ...
        sensitive_fields = [
            'password', 'password_hash', 'mfa_secret', 'api_key', 'private_key',
            'secret', 'token', 'ssn', 'tax_id', 'bank_account_number'
        ]
        sanitized = data.copy()
