...
from ..models.trading import Order, Trade, Portfolio, OrderSide, TradeStatus
from ..models.user import User, UserStatus, KYCStatus
from ..models.carbon_credit import CarbonProject, CarbonCredit, CreditStatus
...
class ComplianceService:
    ...
    def check_order_compliance(self, user_id: int, order_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            position_limits_check = self._check_position_limits_compliance(user_id, order_data)
            ...
            suspicious_activity_check = self._check_suspicious_activity(user_id, order_data)
            ...
            blocking_violations = [c for c in compliance_checks if c.get('blocking', False)]
            ...
            self.audit_service.log_event(
                ...
                metadata={
                    ...
                    'order_value': float(Decimal(str(order_data.get('quantity', 0))) * Decimal(str(order_data.get('price', 0) or 50)))
                },
                success=approved
            )
            ...
            return {
                'approved': approved,
                'reason': blocking_violations[0]['message'] if blocking_violations else None,
                'compliance_checks': compliance_checks,
                'requires_manual_review': any(c.get('manual_review', False) for c in compliance_checks)
            }
    ...
    def _check_kyc_compliance(self, user: User, order_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Check KYC/AML compliance"""
        violations = []
        
        # Basic KYC requirement
        if self.compliance_rules['kyc_required_for_trading'] and not user.is_kyc_approved:
            violations.append({
        ...
        if user.risk_level.value == 'very_high' or user.risk_level.value == 'prohibited':
            violations.append({
    ...
    def _check_trading_hours_compliance(self) -> List[Dict[str, Any]]:
        ...
        # Check trading hours
        if (current_hour_minute < self.trading_hours['market_open'] or 
            current_hour_minute > self.trading_hours['market_close']):
            violations.append({
                'rule': ComplianceRule.TRADING_HOURS,
    ...
    def _check_position_limits_compliance(self, user_id: int, order_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if user and user.risk_level.value == 'low':
                ...
                if daily_volume + order_value > Decimal('50000'):  # $50K daily limit for low-risk users
                    violations.append({
                        'rule': ComplianceRule.POSITION_LIMITS,
    ...
    def _check_market_manipulation(self, user_id: int, order_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Check for potential market manipulation"""
        violations = []
        
        try:
            # Check for unusual price impact
            current_market_price = self._get_current_market_price(order_data.get('credit_type'))
            order_price = Decimal(str(order_data.get('price', 0) or 0))
            
            if current_market_price and order_price > 0:
                price_impact = abs(order_price - current_market_price) / current_market_price
            ...
            if len(recent_orders) > 10:  # Many orders in short time
                cancel_rate = sum(1 for o in recent_orders if o.status.value == 'cancelled') / len(recent_orders)
    ...
    def _check_wash_trading(self, user_id: int, order_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Check for wash trading patterns"""
        violations = []
        
        try:
            # Look for recent opposite trades by the same user
            recent_window = datetime.utcnow() - timedelta(seconds=self.compliance_rules['wash_trading_detection_window'])
            
            opposite_side = 'sell' if order_data.get('side') == 'buy' else 'buy'
            
            recent_opposite_orders = db.session.query(Order).filter(
    ...
    def _check_transaction_velocity(self, user_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            hourly_orders = db.session.query(func.count(Order.id)).filter(
                Order.user_id == user_id,
    ...
    def _check_suspicious_activity(self, user_id: int, order_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if order_value > self.compliance_rules['large_transaction_reporting_threshold']:
                violations.append({
            ...
            if order_value > self.compliance_rules['suspicious_activity_threshold']:
                ...
                if avg_order_value > 0 and order_value > avg_order_value * 10:  # 10x typical size
                    violations.append({
                        'rule': ComplianceRule.AML_SCREENING,
    ...
    def get_user_compliance_status(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            kyc_status = {
                ...
                'verification_level': kyc_record.verification_level if kyc_record else 0,
                'last_updated': kyc_record.updated_at.isoformat() if kyc_record else None,
                'expires_at': kyc_record.expires_at.isoformat() if kyc_record and kyc_record.expires_at else None
            }
            ...
            aml_status = {
                ...
                'last_screened': user.risk_last_assessed.isoformat() if user.risk_last_assessed else None
            }
            
            # Trading Compliance
            trading_compliance = {
                'authorized_for_trading': user.is_kyc_approved and user.status == UserStatus.ACTIVE,
                'daily_volume_used': float(self._get_daily_trading_volume(user_id)),
                'daily_volume_limit': float(self.compliance_rules['max_transaction_velocity'] * 1000),  # Simplified
            ...
            return {
                'user_id': user_id,
                'overall_status': self._determine_overall_compliance_status(compliance_score),
    ...
    def _get_latest_kyc_record(self, user_id: int):
        ...
        return UserKYC.query.filter_by(user_id=user_id).order_by(desc(UserKYC.created_at)).first()
    ...
    def _get_user_average_order_value(self, user_id: int) -> Decimal:
        ...
        avg_value = db.session.query(func.avg(Trade.total_value)).join(
            Order, or_(
                and_(Trade.buy_order_id == Order.id, Order.user_id == user_id),
                and_(Trade.sell_order_id == Order.id, Order.user_id == user_id)
            )
    ...
    def _calculate_compliance_score(self, user: User, kyc_record) -> float:
        ...
        risk_scores = {'low': 20, 'medium': 15, 'high': 10, 'very_high': 5, 'prohibited': 0}
