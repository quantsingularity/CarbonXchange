...
from ..models.trading import Order, Trade, Portfolio, PortfolioHolding, OrderType, OrderSide, OrderStatus, TradeStatus
...
from ..utils import validate_request_data, handle_api_error, paginate_query
...
def create_order():
    ...
    try:
        ...
        # Perform risk checks
        risk_check = risk_service.check_order_risk(user_id, {
        ...
        # Compliance checks
        compliance_check = compliance_service.check_order_compliance(user_id, {
            'order_type': order_type.value,
            'side': side.value,
            'quantity': float(quantity),
            'credit_type': data['credit_type']
        ...
        if not compliance_check['approved']:
            ...
            return jsonify({'error': f'Order rejected: {compliance_check["reason"]}'}), 400
        ...
        if 'expires_at' in data:
            try:
                order_data['expires_at'] = datetime.fromisoformat(data['expires_at'].replace('Z', '+00:00'))
            except ValueError:
        ...
        audit_service.log_event(
            ...
            metadata={'order_id': order.order_id, 'side': side.value, 'quantity': float(quantity)},
            success=True
        )
...
def modify_order(order_id):
    ...
    try:
        ...
        modifiable_fields = ['quantity', 'price', 'stop_price', 'time_in_force', 'expires_at', 'notes']
        ...
        if 'quantity' in data:
            ...
            if order.remaining_quantity <= 0:
                return jsonify({'error': 'New quantity must be greater than filled quantity'}), 400
...
def get_portfolio():
    ...
    try:
        ...
        portfolio_dict['metrics'] = {
            ...
            'diversification_score': trading_service.calculate_diversification_score(holdings),
            'performance_metrics': trading_service.calculate_performance_metrics(user_id, portfolio.id)
        }
