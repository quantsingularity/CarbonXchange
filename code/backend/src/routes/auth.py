...
from ..models.user import User, UserKYC, UserProfile, UserRole, UserStatus
...
def validate_password(password: Any) -> tuple:
    if len(password) < 8:
...
def register() -> Any:
    ...
    try:
        ...
        if not data:
            return (
                jsonify(
                    {"error": "Validation Error", "message": "Request body is required"}
                ),
                400,
            )
...
def get_current_user() -> Any:
    ...
    try:
        ...
        if user.kyc_records:
            latest_kyc = max(user.kyc_records, key=lambda x: x.created_at)
            user_data["kyc"] = latest_kyc.to_dict()
