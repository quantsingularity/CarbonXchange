...
class Order(db.Model):
    ...
    # Order timing
    time_in_force = Column(String(10), nullable=False, default='GTC')  # GTC, IOC, FOK, DAY
...
class Trade(db.Model):
    ...
    # Relationships
    buy_order = relationship("Order", foreign_keys=[buy_order_id], back_populates="trades")
    sell_order = relationship("Order", foreign_keys=[sell_order_id], back_populates="trades")
...
class Portfolio(db.Model):
    ...
    portfolio_type = Column(SQLEnum(PortfolioType), nullable=False, default=PortfolioType.TRADING)
    ...
    def to_dict(self, include_sensitive=False):
        ...
        data = {
            ...
            'created_at': self.created_at.isoformat(),
            'updated_at': self.updated_at.isoformat(),
            'last_valuation_at': self.last_valuation_at.isoformat() if self.last_valuation_at else None
        }
...
class PortfolioHolding(db.Model):
    ...
    # Holding metadata
    vintage_year = Column(Integer, nullable=True)
    acquisition_method = Column(String(50), nullable=True)  # purchase, transfer, issuance
    
    # Timestamps
    created_at = Column(DateTime, nullable=False, default=lambda: datetime.now(timezone.utc))
    updated_at = Column(DateTime, nullable=False, default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))
    first_acquired_at = Column(DateTime, nullable=False, default=lambda: datetime.now(timezone.utc))
    ...
    def to_dict(self, include_sensitive=False):
        ...
        data = {
            ...
            'last_traded_at': self.last_traded_at.isoformat() if self.last_traded_at else None
        }
