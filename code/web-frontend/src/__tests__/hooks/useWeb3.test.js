...
describe('useWeb3 Hook', () => {
    ...
    describe('connectWallet', () => {
        it('connects to wallet and sets account and balance successfully', async () => {
            ...
            expect(result.current.balance).toBe(Web3.utils.fromWei(mockBalance, 'ether'));
            expect(result.current.error).toBeNull();
        ...
        it('handles error if window.ethereum is not present', async () => {
            ...
            expect(result.current.loading).toBe(false);
            expect(result.current.error).toBe('Please install MetaMask or a Web3-enabled browser.');
            expect(result.current.account).toBeNull();
            expect(result.current.balance).toBe('0');
    ...
    describe('getContractInstance', () => {
        ...
        it('returns null if web3 is not initialized', () => {
            ...
            const contractInstance = result.current.getContractInstance([], '0xAddress');
            expect(contractInstance).toBeNull();
            expect(result.current.error).toBe('Web3 instance not available. Connect wallet first.');
        });
    });
