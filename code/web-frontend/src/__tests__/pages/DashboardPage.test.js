...
jest.mock('../../../hooks/useApi', () => ({
    __esModule: true,
    default: () => ({
        get: jest.fn(() => Promise.resolve({ data: { mockData: 'dashboard data' } })),
        // Mock other methods like post, put, delete if used by the page
    }),
}));

jest.mock('../../../components/CreditVisualization', () => () => <div data-testid="mock-credit-visualization">Mock Credit Visualization</div>);
jest.mock('../../../components/MarketAnalysis', () => () => <div data-testid="mock-market-analysis">Mock Market Analysis</div>);
jest.mock('../../../components/TradingDashboard', () => () => <div data-testid="mock-trading-dashboard">Mock Trading Dashboard</div>);
