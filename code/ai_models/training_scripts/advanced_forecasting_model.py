...
class FeatureEngineer:
    ...
    def create_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        df['log_price'] = np.log(df['price'] + 1e-8)  # Avoid log(0)
        ...
        # External factors (mock data - would be real economic indicators)
        df['economic_indicator_1'] = np.random.normal(0, 1, len(df))  # GDP growth proxy
        df['economic_indicator_2'] = np.random.normal(0, 1, len(df))  # Interest rate proxy
        df['environmental_factor'] = np.random.normal(0, 1, len(df))  # Environmental policy proxy
        
        # Remove infinite and NaN values
        df = df.replace([np.inf, -np.inf], np.nan)
        
        # Store feature names (excluding target and timestamp)
        self.feature_names = [col for col in df.columns if col not in ['price', 'timestamp', 'target']]
...
class AdvancedForecastingModel:
    ...
    def prepare_data(self, data_path: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            # Create target variable (next period price)
    ...
    def _generate_synthetic_data(self, n_samples: int = 1000) -> pd.DataFrame:
        ...
        seasonal = 10 * np.sin(2 * np.pi * np.arange(n_samples) / 24)  # Daily seasonality
        ...
        data = pd.DataFrame({
            ...
            'price': prices,
            'volume': volumes,
            'bid_price': prices * 0.995,  # Slightly lower bid
            'ask_price': prices * 1.005,  # Slightly higher ask
        })
    ...
    def train_models(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, ModelPerformance]:
        ...
        for model_name, model in models_config.items():
            ...
            try:
                ...
                cv_scores = cross_val_score(model, X_selected, y, cv=tscv, scoring='neg_mean_squared_error')
        ...
        if len(self.models) >= 2:
            ...
            performance_results['ensemble'] = ModelPerformance(
                ...
                r2=r2_ensemble,
                mape=mape_ensemble,
                directional_accuracy=0.0,  # Not calculated for ensemble
                training_time=0.0,
                prediction_time=0.0
            )
            
            logger.info(f"Ensemble - MSE: {mse_ensemble:.4f}, MAE: {mae_ensemble:.4f}, R²: {r2_ensemble:.4f}")
    ...
    def predict(self, X: pd.DataFrame, model_name: str = 'ensemble') -> List[ForecastResult]:
        ...
        for i, pred in enumerate(predictions):
            result = ForecastResult(
                ...
                model_confidence=0.8,  # Simplified confidence score
                contributing_factors={}  # Would include feature importance
            )
            results.append(result)
        
        # Update prediction time in performance metrics
    ...
    def load_models(self, model_dir: str):
        ...
        try:
            ...
            for model_file in os.listdir(model_dir):
                if model_file.endswith('_model.pkl') and model_file != 'ensemble_model.pkl':
            ...
            # Load scalers and feature selector
            scalers_path = os.path.join(model_dir, "scalers.pkl")
...
def main():
    ...
    data_path = os.path.join(
        ...
        'resources', 'datasets', 'market_demand.csv'
    )
