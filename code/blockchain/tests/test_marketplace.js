...
describe('CarbonCreditMarketplace', function () {
    ...
    beforeEach(async function () {
        ...
        CarbonCreditMarketplace = await ethers.getContractFactory('CarbonCreditMarketplace');
    ...
    describe('Listing Credits', function () {
        ...
        it('Should allow a seller to list credits', async function () {
            ...
            expect(await carbonCreditToken.balanceOf(await marketplace.getAddress())).to.equal(
                creditAmount,
            );
    ...
    describe('Buying Credits', function () {
        ...
        it('Should allow a buyer to purchase credits', async function () {
            ...
            const initialBuyerCCTBalance = await carbonCreditToken.balanceOf(buyer.address);
            ...
            const finalSellerEthBalance = await ethers.provider.getBalance(seller.address);
    ...
    describe('Cancelling Listings', function () {
        ...
        it('Should allow the seller to cancel their own listing', async function () {
            ...
            const initialSellerCCTBalance = await carbonCreditToken.balanceOf(seller.address);
            ...
            expect(await carbonCreditToken.balanceOf(await marketplace.getAddress())).to.equal(
                initialMarketplaceCCTBalance - creditAmount,
            );
        ...
        it('Should fail if a non-seller tries to cancel the listing', async function () {
            const listingId = 1;
            await expect(marketplace.connect(buyer).cancelListing(listingId)).to.be.revertedWith(
                'Marketplace: Only seller can cancel listing',
            );
        });
