...
const TradeHistoryScreen = ({ navigation }) => {
  ...
  const fetchUserTrades = useCallback(async (pageNum = 1, refreshing = false) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || err.message || 'An error occurred while fetching trade history';
      if (pageNum === 1) {
          setTrades([]);
  ...
  }, [trades]); // Include trades for appending logic
  ...
  const renderTradeItem = ({ item }) => (
    <View style={styles.tradeItemContainer}>
      <View style={styles.tradeItemHeader}>
        <Text style={[styles.tradeItemType, item.type === 'buy' ? styles.buyType : styles.sellType]}>
          {item.type?.toUpperCase() || 'N/A'}
        </Text>
        <Text style={styles.tradeItemDate}>{new Date(item.createdAt).toLocaleString()}</Text>
      </View>
      <Text style={styles.tradeItemDetail} numberOfLines={1} ellipsizeMode="tail">Credit: {item.creditId || 'N/A'}</Text>
