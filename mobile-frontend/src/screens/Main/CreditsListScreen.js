...
const CreditsListScreen = ({ navigation }) => {
    ...
    const fetchCredits = useCallback(
        async (pageNum = 1, refreshing = false, search = '') => {
        ...
        [credits],
    ); // Include credits for appending logic
        ...
        >
            <View style={styles.itemHeader}>
                <Text style={styles.itemType}>
                    {item.type ? item.type.replace(/_/g, ' ').toUpperCase() : 'Unknown Type'}
                </Text>
                <Text
                    style={[
                        styles.itemStatus,
                        item.status === 'available' ? styles.statusAvailable : styles.statusOther,
                    ]}
                >
                    {item.status || 'N/A'}
                </Text>
            ...
            <View style={styles.itemRow}>
                ...
                <Text style={styles.itemValue}>
                    {item.amount !== undefined ? `${item.amount} tCO2e` : 'N/A'}
                </Text>
            </View>
            <View style={styles.itemRow}>
